...
def get_keys(db):
    ...
    props = db.Properties.find({
        '$and':[
            ...
            {'$or':[
                {'Disappered': {'$exists': False}},
                {'Disappered': False},
            ]},
            {'$or':[
                {'AutoInquiredSuccessTime': {'$exists': False}},
                {'AutoInquiredSuccessTime': {'$lt': datetime.now() - timedelta(days=30)}},
            ]}
        ]
    })
    ...
    for prop in props:
        ...
        # 1度も自動照会されていない
        if key['Priority'] == 300 and not 'AutoInquiredTime' in prop:
            key['Priority'] = 400

        # 手続の完了から1～3ヶ月
        if not 'AutoInquiredSuccessTime' in prop or prop['AutoInquiredSuccessTime'] < (datetime.now() - timedelta(days=20)):
        ...
        # 40日以内に照会したものはスキップ
        if key['Priority'] <= 300 and prop['AutoInquiredTime'] >= (datetime.now() - timedelta(days=40)):
            continue
...
def inquire_all(db):
    ...
    for key in get_keys(db):
        ...
        if data is None:
            logger.warning('patent infomation is not found. (%s, %s)', key['Law'], key['RegistrationNumber'])
            if msg:
                logger.warning(msg)
