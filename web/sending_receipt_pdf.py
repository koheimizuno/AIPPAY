...
def make(req_id, prop_id):
    ...
    if lang.name == 'ja':
        #font_name = 'ipag.ttf'
        font_name = 'yumin.ttf'
    elif lang.name == 'zh_CN':
        font_name = 'MicrosoftJhengHeiRegular.ttf'
    elif lang.name == 'zh_TW':
        font_name = 'MicrosoftJhengHeiRegular.ttf'
    ...
    with PdfMaker(font_file_name=font_name) as pdf:
        ...
        # 前文
        y += pdf.put_text(65, y, lang['ReceiptMail']['Preamble'], wrap_cols=44, line_height=line_height) * line_height
        y += 3
        y += pdf.put_text(pdf.width - 65, y, lang['ReceiptMail']['Preamble_BR'], align_right=True) * line_height
        y += line_height

        # 納付の内容
        pdf.put_text(pdf.width / 2, y, lang['ReceiptMail']['ItemTitle'], align_center=True)
        y += line_height * 1.5
        ...
        # 添付書類
        y += pdf.put_text(80, y, '{}    {}'.format(lang['ReceiptMail']['AttachmentTitle'], re.sub(r'\t', ' ', lang['ReceiptMail']['AttachmentName'])), wrap_cols=(pdf.width - 1)) * line_height

        # フッター
        if lang['ReceiptMail']['Footer'].strip()  != "":
            y += pdf.put_text(80, y, lang['ReceiptMail']['Footer'], wrap_cols=(pdf.width - 1)) * line_height
        pdf.put_text(pdf.width - 80, y, lang['ReceiptMail']['Footer_BR'], align_right=True)

        # ファイル名
        fname = '{:%Y%m%d}_{}{}_領収書の送付.pdf'.format(
            datetime.now(),
            lang['Law'][prop['Law']],
...
if __name__ == '__main__':
    ...
    with open('log/sending_receipt.pdf', 'wb') as fout:
        fout.write(rep['Raw'])
